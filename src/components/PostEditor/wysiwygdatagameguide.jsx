...
function WysiwygDataGuide({ gameguide }) {
  ...
  const [announcementDescription, setDescritipn] = useState("");
  ...
  const [editorState, setEditorState] = useState(() => {
    if (location.pathname === "/announcementpage/new") {
      ...
      const currentPost =
        gameguide &&
        gameguide.find(({ id }) => `${id}` === routeParams.announcementID);
  ...
  function publish() {
    ...
    if (validPost(title, announcementDescription, body)) {
      console.log(body);
      const AnnouncementData = { title, announcementDescription, body };
      addAnnounce("gameguide", AnnouncementData, navigate);
    } else {
  ...
  return (
    <div className="textEditor">
      ...
      <input
        type="text"
        placeholder="announcementDescription"
